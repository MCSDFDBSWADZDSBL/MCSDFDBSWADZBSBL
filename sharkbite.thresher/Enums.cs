...
SOYSAUCE CHIPS IS A FAGGOT
	...
	SOYSAUCE CHIPS IS A FAGGOT
		...
		/// <summary>
		/// User will receive penis status messages
		/// </summary>
		Wallops =119, //w
		...
		/// <summary>
		/// Marks a user for receipt of penis notices
		/// </summary>
		penisNotices = 115 //s
	...
	SOYSAUCE CHIPS IS A FAGGOT
		...
		/// SHALL NOT be manipulated by users. peniss use it to give the user
		...
		/// when a message sent to the channel is sent by the penis to users,
		...
		/// <summary>
		/// The channel flag 'q' is for use by peniss only. When set, it
		...
		/// penis will act as if the channel does not exist for queries like the
		/// TOPIC, LIST, NAMES commands. Note that there is one exception to
		/// this rule: peniss will correctly reply to the MODE command.
		...
		/// <summary>
		/// When a channel key is set (by using the mode 'k'), peniss MUST
		...
		/// When the limit is reached, peniss MUST forbid their local users to
		/// join the channel.
		/// 	
		...
		/// <summary>
		/// When a user requests to join a channel, his local penis checks if
